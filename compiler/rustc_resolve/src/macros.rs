...
impl<'ra, 'tcx> ResolverExpand for Resolver<'ra, 'tcx> {
    ...
    fn expansion_for_ast_pass(
        ...
        let parent_module_resolved =
            parent_module.map_or(self.empty_module, |def_id| self.expect_module(def_id));
        self.ast_transform_scopes.insert(expn_id, parent_module_resolved);

        // CRITICAL FIX: Also set up invocation_parent_scopes and invocation_parents entries
        // This is needed for DefCollector when fully_expand_fragment is called
        // on fragments generated by AstPass transformations
        let parent_def = parent_module.map_or(crate::CRATE_DEF_ID, |def_id| {
            // Convert DefId to LocalDefId - for crate-local items this is safe
            LocalDefId { local_def_index: def_id.index }
        });

        let invocation_parent = crate::InvocationParent {
            parent_def,
            impl_trait_context: crate::ImplTraitContext::Existential,
            in_attr: false,
        };

        // Build proper ParentScope struct for invocation_parent_scopes
        let parent_scope = ParentScope {
            module: parent_module_resolved,
            expansion: expn_id,
            macro_rules: self.arenas.alloc_macro_rules_scope(MacroRulesScope::Empty),
            derives: &[],
        };

        self.invocation_parent_scopes.insert(expn_id, parent_scope);
        self.invocation_parents.insert(expn_id, invocation_parent);
